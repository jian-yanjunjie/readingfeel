		...
		return this.each(function(){
			...
			this.createAgoCommentHtml = function(){
				...
				$.each(para.agoComment, function(k, v){
					...
					item += '			<a class="reply" href="javascript:void(0)" selfID="'+v.id+'" sortID="'+v.sortID+'" >回复</a>';
					item += '		</div>';
					item += '	</div>';
					item += '</div>';
...
//						if($("#comment"+v.sortID).find(".comments").length==0){  // 没有
						if($("#comment"+v.pldx).find(".comments").length==0){  // 没有
							var comments = '';
							comments += '<div id="comments'+v.sortID+'" class="comments">';
							comments += 	item;
							comments += '</div>';
							$("#comment"+v.pldx).append(comments);
						}else{  // 有
							$("#comments"+v.pldx).append(item);
						}
					}
				});
			 ...
			 * 返回: 无
			 */
			this.replyClickEvent = function(pldx){
				// 绑定回复按钮点击事件
				$(self).find(".actions .reply").live("click", function(){
					// 设置当前回复的评论的id
					fCode = $(this).attr("selfID");
					var sortID = 1+parseInt($(this).attr("sortID"));
					// 1.移除之前的取消回复按钮
...
function getnowtime(){
	 ...
	 var mydate = (year.toString()+"-"+month.toString()+"-"+day.toString()+" "+hh.toString()+":"+mm.toString());
	 return mydate;
}
